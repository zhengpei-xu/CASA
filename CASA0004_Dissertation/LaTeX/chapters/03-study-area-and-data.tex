...
The analytical pipeline draws on four principal data sources. The primary source is Ordnance Survey National Geographic Database Transport (OS NGD Transport), which provides network geometry, road classification, carriageway and cycle lane width, legal access designation, junction type and probe-vehicle-derived average speeds for constructing the directed, stress-classified network (Sections 4.1–4.3). The Cycle Lane feature collection, achieving full national coverage in March 2026 \parencite{ordnancesurvey2026e}, makes this study one of the first to incorporate authoritative national cycling infrastructure data into a network-level assessment. Department for Transport road traffic count data supply observed AADT volumes and training observations for minor road volume estimation (Section 4.2). OpenStreetMap (OSM) contributes cycling geometries for Path Link cross-validation, traffic signal locations for junction classification, and school locations for accessibility analysis (Sections 4.1, 4.3, 4.4). Census data from the Office for National Statistics provide MSOA-level commuting flows for OD connectivity and network growth evaluations, and LSOA-level population and boundaries for density and reach analyses (Sections 4.4–4.5). Table~\ref{tab:datasets} summarises each dataset.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l l X l l}
    \toprule
    \textbf{Dataset} & \textbf{Provider} & \textbf{Key Variables} & \textbf{Spatial Unit} & \textbf{Used In} \\
    \midrule
    Road Links        & OS NGD & Geometry, classification, description, width & Edge & 4.1--4.3 \\
    Path Links        & OS NGD & Geometry, cycle lane presence & Edge & 4.1 \\
    Highway Dedication & OS NGD & Legal access designation & Edge & 4.1 \\
    Cycle Lane        & OS NGD & Facility type, width, placement side & Edge & 4.1--4.2 \\
    Average Speed (RAMI) & OS NGD & Probe-derived average speed & Edge & 4.2 \\
    Road Node         & OS NGD & Node form, geometry, description & Node & 4.3 \\
    Path Node         & OS NGD & Node geometry & Node & 4.3 \\
    \midrule
    Road Traffic Counts & DfT  & AADT by vehicle type & Count point & 4.2 \\
    \midrule
    Cycling Network   & OSM   & Highway and cycleway tags & Way & 4.1 \\
    Traffic Signals   & OSM   & Signal node locations & Node & 4.3 \\
    Schools           & OSM   & Location & Point & 4.4 \\
    \midrule
    Commuting Flows   & ONS   & OD pairs within 8\,km & MSOA & 4.4--4.5 \\
    Population        & ONS   & Resident population & LSOA & 4.4 \\
    Boundaries        & ONS   & LSOA/MSOA polygons & Polygon & 4.4--4.5 \\
    \bottomrule
  \end{tabularx}
\end{table}